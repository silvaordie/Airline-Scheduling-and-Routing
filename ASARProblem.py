...
class ASARProblem(Problem):
    ...
    def actions(self, state):
        ...
        for leg in legs:
            for i in range(len(leg.airplanes)):
                if (state.locations[leg.airplanes[i]].name == leg.departure.name && Time.later(leg.arrival.closingTime, Time.add( leg.duration , airplanes[leg.airplanes[i]].rotationTime ))):
    ...
    def save(f,s):
        for k in range(len(states[0].locations))

            string = "S " + airplanes[k].name + " "
            prev = None
            for s in states:
                if prev != s.locations[k]
                    if prev != None:
                        string = string + s.times[k].toString() + " "
                        string = string + s.locations[k].name + " "  
                    else:
                        string = string + s.locations[k].name + " "  
                        string = string + (s.times[k].toString()) + " "
                        string = string + s.locations[k].name + " "  
                    prev =  s.locations[k].name
